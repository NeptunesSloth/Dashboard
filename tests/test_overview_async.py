...
def _client(monkeypatch, async_on: bool):
    ...
    monkeypatch.setattr(app_mod, "all_devices", lambda: [{"name": "d1", "url": "http://x"}])
    monkeypatch.setattr(app_mod, "aggregate", lambda devices: {**FAKE, "via": "sync"})

    async def fake_async(devices):
        return {**FAKE, "via": "async"}

    monkeypatch.setattr(app_mod.aggregator, "aggregate_async", fake_async)
...
def test_async_falls_back_to_sync_when_tunneled(monkeypatch):
    client, app_mod = _client(monkeypatch, async_on=True)
    # A device on the reverse tunnel forces the sync (direct-HTTP-safe) path.
    monkeypatch.setattr(app_mod, "_any_tunneled", lambda devices: True)
