...
- **Pills & elixirs:** spend a disciple's **spirit stones** on temporary buffs — *Enlightenment Elixir* (ascend to a stronger model), *Qi-Gathering Pill* (deeper responses), *Foundation Pill* (+autonomy budget), *Heart-Demon Pill* (a harsher inner demon). Concoct from the disciple's card; buffs are time-limited. `GET /api/pills`, `POST /api/pills/buy`.
