...
- **Spirit-stone economy:** all rewards/penalties are env-tunable (`MAYBOT_AWARD_*`, `MAYBOT_PENALTY_FAIL`, `MAYBOT_TRIBULATION_*`), and a **daily stipend** (`MAYBOT_STIPEND`, default 15, every `MAYBOT_STIPEND_HOURS`) tops up every disciple so cultivation never fully stalls.
