...
- **Cultivation (reward & punishment):** disciples (agents) earn **spirit stones** for good work — completing tasks, mastering a new **technique** (succeeding at a tool for the first time), and joining the Dao Council. With enough spirit stones *and* techniques they **break through** to the next cultivation realm (Qi Condensation → Foundation Establishment → Core Formation → … → Immortal Ascension). Failures chip away stones; a streak of them calls down a **heavenly tribulation** that strips stones and can strike a disciple **down a realm** (qi deviation). Shown on each disciple card (realm · stage, qi bar, techniques) with breakthrough/tribulation flourishes; `GET /api/cultivation`; persisted via `MAYBOT_DB`.
