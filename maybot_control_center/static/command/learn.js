...
function selectTrack(id) {
  ...
  $('chat').innerHTML = '';
  renderTracks(); renderTopics();
  $('work').innerHTML = `<div class='muted'>Choose a topic to begin a lesson${curTrack && curTrack.labs.length ? ', or jump into a hands-on lab below.' : '.'}</div>`
    + labButtons();
  bindLabButtons();
...
async function startLab(kind) {
  ...
  $('work').innerHTML = `<div style='font-size:13.5px; margin-bottom:10px'><b>Brief:</b> ${esc(res.brief)}</div>
    ...
    <textarea class='finding' id='finding' placeholder='${kind === 'ids' ? 'Describe the intrusion you found and your evidence…' : 'Report the vulnerability, how you would exploit it, and the flag…'}'></textarea>
